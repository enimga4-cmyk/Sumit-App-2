...
export function generateSubjectPdfReport(student: Student, subject: string, notes: ChapterNote[]) {
  ...
  doc.setTextColor(71, 85, 105);
  doc.text("Ch #", 16, 132);
  doc.text("Chapter / Note Name", 30, 132);
  doc.text("Status", 110, 132);
  doc.text("Student Notes / Remarks", 135, 132);
  ...
  if (notes.length === 0) {
    ...
    notes.forEach((note) => {
      ...
      if (chapName.length > 35) {
        chapName = chapName.substring(0, 32) + "...";
      }
      doc.text(chapName, 30, y);
      ...
      if (note.isCompleted) {
        ...
        doc.setFont("helvetica", "bold");
        doc.text("IN PROGRESS", 110, y);
      }
      doc.setFont("helvetica", "normal");
      doc.setTextColor(71, 85, 105);

      let remarkText = note.remark || "No remarks added.";
      if (remarkText.length > 32) {
        remarkText = remarkText.substring(0, 29) + "...";
      }
      doc.text(remarkText, 135, y);
